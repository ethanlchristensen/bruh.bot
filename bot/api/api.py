...
async def update_config(updates: UpdateConfigRequest, authorized: bool = Depends(verify_admin)):
    ...
    try:
        ...
        return ConfigResponse(success=True, message="Config updated", version=config._version)
...
async def reload_config(authorized: bool = Depends(verify_admin)):
    ...
    try:
        ...
        return ConfigResponse(success=True, changed=changed, version=config._version, message="Config reloaded" if changed else "No changes detected")
...
async def update_ai_provider(data: UpdateAIProviderRequest, authorized: bool = Depends(verify_admin)):
    """Update AI provider settings."""
    try:
        config = get_config_service()

        updates = {"aiConfig": config.dynamic.aiConfig.model_dump()}
        ...
        return ConfigResponse(success=True, message=f"AI provider updated to {data.provider}", version=config._version)
...
async def add_admin(data: AddAdminRequest, authorized: bool = Depends(verify_admin)):
    """Add admin user ID."""
    try:
        config = get_config_service()
        ...
        return ConfigResponse(success=True, message=f"Added admin {data.userId}", version=config._version)
...
async def remove_admin(user_id: str, authorized: bool = Depends(verify_admin)):
    ...
    try:
        ...
        return ConfigResponse(success=True, message=f"Removed admin {user_id}", version=config._version)
...
async def get_version(authorized: bool = Depends(verify_admin)):
    ...
    config = get_config_service()
    return {"version": config._version, "lastUpdated": config.dynamic.lastUpdated}
...
if __name__ == "__main__":
    ...
    uvicorn.run("config_api:app", host="0.0.0.0", port=port, reload=False, log_level="info")
