...
PENALTIES = {"interaction_spam": {1: 1, 2: 2, 3: 3}, "bot_targeted_abuse": {1: 2, 2: 3, 3: 5}, "targeted_harassment": {1: 3, 2: 5, 3: 8}, "threat_or_intimidation": {1: 5, 2: 8, 3: 10}, "block_evasion": {1: 2, 2: 3, 3: 5}}
...
class MongoReputationService:
    ...
    async def get_profile(self, guild_id: int, user_id: int) -> dict:
        ...
        await self.collection.update_one(
            ...
            {"$setOnInsert": {"guild_id": Int64(guild_id), "user_id": Int64(user_id), "score": 0, "status": "active", "blocked_until": None, "last_notice_at": None, "created_at": now}},
            upsert=True,
        )
    ...
    async def record_event(self, guild_id: int, user_id: int, source_message_id: int, channel_id: int, reason_code: str, severity: int, confidence: float, summary: str) -> dict:
        if user_id == self.bot.user.id or reason_code not in PENALTIES or not 1 <= severity <= 3:
        ...
        delta = PENALTIES[reason_code][severity]
        now = datetime.now(UTC)
