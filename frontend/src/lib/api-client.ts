...
export interface DynamicConfig {
  ...
  allowedBotsToRespondTo: number[];
  deleteUserMessages: DeleteUserMessagesConfig;
  globalBlockList: number[];
}
...
export class ConfigAPIClient {
  ...
  async addToBlockList(userId: number): Promise<ConfigResponse> {
  ...
  async removeFromBlockList(userId: number): Promise<ConfigResponse> {
    const config = await this.getConfig();
    const currentList = config.config?.globalBlockList || [];
  ...
  async addToCooldownBypass(userId: number): Promise<ConfigResponse> {
    const config = await this.getConfig();
  ...
  async removeFromCooldownBypass(userId: number): Promise<ConfigResponse> {
    const config = await this.getConfig();
    const currentList = config.config?.cooldownBypassList || [];
