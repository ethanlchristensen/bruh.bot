...
import { Save, Sparkles, BrainCircuit, FileText, Server, Eye, EyeOff } from 'lucide-react';
...
function AIConfigComponent() {
  ...
  useEffect(() => {
    if (data?.config && !isSaving) {
      ...
      setImageGenModel(config.aiConfig.imageGeneration?.preferredModel || '');
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-border/40 pb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Configure LLM providers, models, and bot personality.
          </p>
        </div>
          ...
          <Card className="border-primary/20 shadow-md">
            <CardHeader className="flex flex-row items-center gap-4">
              ...
              <div>
                ...
                <CardDescription>Configure preferred provider and model for conversations.</CardDescription>
              </div>
            </CardHeader>
            <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button
                    ...
                    className={`flex flex-col items-center justify-center gap-3 p-4 rounded-xl border-2 text-center transition-all ${
                      preferredProvider === 'ollama' ? 'border-primary bg-primary/5 text-primary shadow-sm' : 'border-border/60 hover:border-border hover:bg-muted/40 text-muted-foreground'
                    }`}
                  >
                    <Ollama size={40} />
                    <div>
                  ...
                  <button
                    ...
                    className={`flex flex-col items-center justify-center gap-3 p-4 rounded-xl border-2 text-center transition-all ${
                      preferredProvider === 'openrouter' ? 'border-primary bg-primary/5 text-primary shadow-sm' : 'border-border/60 hover:border-border hover:bg-muted/40 text-muted-foreground'
                    }`}
                  >
                    <OpenRouter size={40} />
                    <div>
              ...
              <div className="space-y-2 border-t border-border/40 pt-4">
                <Label className="font-semibold">Image Generation Model (OpenRouter)</Label>
                <ModelSelector
              ...
              <div className="grid gap-4 sm:grid-cols-2 border-t border-border/40 pt-4">
                ...
                <div className="space-y-2 p-3 rounded-lg bg-muted/40">
                  ...
                  <Input type="number" value={maxDailyImages} onChange={(e) => setMaxDailyImages(parseInt(e.target.value) || 5)} className="h-8 max-w-[120px]" min={1} />
                </div>
              </div>
            </CardContent>
          </Card>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button
                    ...
                    className={`flex flex-col items-center justify-center gap-3 p-4 rounded-xl border-2 text-center transition-all ${
                      orchestratorProvider === 'ollama' ? 'border-primary bg-primary/5 text-primary shadow-sm' : 'border-border/60 hover:border-border hover:bg-muted/40 text-muted-foreground'
                    }`}
                  >
                    <Ollama size={40} />
                    <div><div className="font-semibold text-foreground">Ollama</div><div className="text-xs">Self-hosted local models</div></div>
                  </button>
                  <button
                    onClick={() => setOrchestratorProvider('openrouter')}
                    className={`flex flex-col items-center justify-center gap-3 p-4 rounded-xl border-2 text-center transition-all ${
                      orchestratorProvider === 'openrouter' ? 'border-primary bg-primary/5 text-primary shadow-sm' : 'border-border/60 hover:border-border hover:bg-muted/40 text-muted-foreground'
                    }`}
                  >
                    <OpenRouter size={40} />
                    <div><div className="font-semibold text-foreground">OpenRouter</div><div className="text-xs">Any LLM cloud endpoint</div></div>
                  </button>
                </div>
              </div>
              <div className="space-y-2 border-t border-border/40 pt-4">
              ...
              <div className="space-y-4 p-4 rounded-xl border border-primary/10 bg-primary/5">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <div className="flex gap-1.5 ml-auto">
                    ...
                    {orchestratorProvider === 'ollama' && <span className="text-[9px] bg-purple-500/20 text-purple-600 dark:text-purple-400 px-1.5 py-0.5 rounded font-bold uppercase tracking-wider">Orch Active</span>}
                  </div>
                </div>
                <div className="space-y-2">
                  <Label>Ollama Server Endpoint</Label>
                  <Input value={ollamaEndpoint} onChange={(e) => setOllamaEndpoint(e.target.value)} placeholder="e.g. http://localhost:11434" />
                </div>
                <div className="space-y-2">
                  <Label>Ollama Default Model</Label>
                  <ModelSelector
              ...
              <div className="space-y-4 p-4 rounded-xl border border-primary/10 bg-primary/5">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <div className="flex gap-1.5 ml-auto">
                    ...
                    {orchestratorProvider === 'openrouter' && <span className="text-[9px] bg-purple-500/20 text-purple-600 dark:text-purple-400 px-1.5 py-0.5 rounded font-bold uppercase tracking-wider">Orch Active</span>}
                  </div>
                </div>
                <div className="space-y-2">
                  <Label>OpenRouter API Key</Label>
                  <div className="relative">
                    <Input type={showApiKey ? 'text' : 'password'} value={openrouterApiKey} onChange={(e) => setOpenrouterApiKey(e.target.value)} placeholder="sk-or-..." className="pr-10" />
